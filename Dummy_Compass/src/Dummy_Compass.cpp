...
bool isStateValid(const oc::SpaceInformation *si, const ob::State *state)
{
    ...
    // gap crossing
    bool gap_checkR = posR[0] < -GAP_LENGTH/2-0.05 || posR[0] > GAP_LENGTH/2+0.05;
    bool gap_checkL = posL[0] < -GAP_LENGTH/2-0.05 || posL[0] > GAP_LENGTH/2+0.05;
...
void plan()
{
    ...
    if (solved)
    {
		printf("Found solution in %i iteration: %i", n, solved);
