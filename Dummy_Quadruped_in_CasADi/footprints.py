...
f_init = np.array([0., 0., 250., 0., 0., 250., 0., 0., 250., 0., 0., 250.])
...
I3 = SX.eye(3)
I3cat = horzcat(I3, I3)
tau_CS = SX.sym('tau_CS', 12)

# Front
tau_CSF = SX.sym('tau_CSF', 6)
deltaF = p[6:12] - vertcat(comF, comF)
mF = 50.

S1F = skew(deltaF[0:3])
S2F = skew(deltaF[3:6])

SFcat = horzcat(S1F, S2F)
G_CDF = vertcat(I3cat, SFcat)

tau_CSF = mF * g_vec + mtimes(G_CDF, f[6:12])

# Back
tau_CSB = SX.sym('tau_CSB', 6)
deltaB = p[0:6] - vertcat(comB, comB)
mB = 50.

S1B = skew(deltaB[0:3])
S2B = skew(deltaB[3:6])

SBcat = horzcat(S1B, S2B)
G_CDB = vertcat(I3cat, SBcat)

tau_CSB = mB * g_vec + mtimes(G_CDB, f[0:6])


tau_CS = vertcat(tau_CSB, tau_CSF) 
...
blue = tuple(np.array([0., 0., 255.])/255.)
...
colors = [blue, red, grey, green, brown, purple, orange, dark_green, pink, black]
...
for i in range(0, ns):
    ax = fig2.add_subplot(2, 5, i+1, projection='3d')
